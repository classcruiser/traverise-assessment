    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-grey-800">
                <h5 class="modal-title">Addon details</h5>
...
<script>
        ...
        if (v.startsWith(url, '{{ url('/') }}')) {
            axios.get(url +'?addons').then(res => {
                ...
                if (v.countSubstrings(data, 'input')) {
                    ...
                    swal({
                        text: "There is no addon need to be processed!",
                        icon: "error",
                        button: "Rescan",
                    }).then(() => {
                        resumeQRScanner();
    ...
    $(document).on('click', '.scan-again', function (e) {
        ...
        $("#result input[name='guests']:checked").each(function () {
            ...
            const strAddon = $(`#result #addon${guestId}`).val();
            const addonIds = v.split(strAddon, ',');
            selected.push({ id: guestId, addons: addonIds});
        });

        if (selected.length > 0) {
            $('.scan-again').attr('disabled', true);

            const postUrl = '{{ url("bookings/check-in/addon") }}';
