...
<div class="min-h-screen w-full relative" style="background-color: {{ $profile->bg_color }}">
	<div class="w-full md:w-[720px] mx-auto py-0 md:py-10">
		@if ($has_checked_in)
			<div class="border-0 md:border border-solid border-lime-200 bg-lime-50 p-3 text-lime-700 text-lg mb-4">
		...
		<div class="border-t-4 border-solid" style="border-color: {{ $profile->accent_color }}; background-color: {{ $profile->secondary_color }}">
			...
			<div class="bg-gray-50">
				<div class="grid grid-cols-12 gap-4 py-5 px-5">
					...
					<div class="col-span-12 md:col-span-6">
						<span class="block uppercase text-xs tracking-wide text-gray-400">BOOKER</span>
						<span class="text-2xl">
							{{ $booking->guest->details->full_name }}
                            <i class="fa fa-check-circle text-green-500 ml-1"></i>
						</span>
					</div>
				</div>
			</div>
			@foreach ($booking->sessions as $session)
				<div class="bg-white border-b border-gray-200">
					...
					<table class="table w-full text-sm">
						<thead>
							<tr>
								<th class="w-1/2 bg-gray-50 text-left py-3 px-6 uppercase">Class / Session</th>
								<th class="w-1/2 bg-gray-50 text-right py-3 px-6 uppercase">Date</th>
							</tr>
						</thead>
						<tbody>
                            <tr>
                                <td class="py-3 px-6" valign="top">{{ $session->session->name }}</td>
                                <td class="py-3 px-6 text-right">{{ $session->date->format('l, d M y') }}, {{ $session->schedule->start_formatted }} - {{ $session->schedule->end_formatted }}</td>
                            </tr>
						</tbody>
					</table>
				</div>
